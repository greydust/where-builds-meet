...
try {
  ...
  const before = (value) => round(value - 0.0001);
  const after = (value) => round(value + 0.0001);
  ...
  for (const path of rotationPaths) {
    ...
    const firstActionTime = Math.min(...skillRows.flatMap((row) => row.actions.map((action) => row.startTime + Number(action.time ?? 0))));
    const firstFleetingTrace = skillRows.find((row) => row.step.skill === "SnowpartingSpecial");
    const moves = [
      { reason: "before first action", distance: 19, startTime: before(relative(firstActionTime)) },
      { reason: "before first Fleeting Trace", distance: 3, startTime: before(relative(firstFleetingTrace.startTime)) },
      { reason: "after first Fleeting Trace", distance: 1, startTime: after(relative(firstFleetingTrace.startTime + firstFleetingTrace.effectiveCastTime)) },
    ];
    ...
    for (const row of skillRows.filter((candidate) => candidate.step.skill === "PhalanxbaneHeavyCharged3")) {
      ...
      moves.push(
        { reason: `before first Anxi (${row.rotationIndex})`, distance: 6, startTime: before(relative(row.startTime + Number(row.actions[firstAnxi]?.time ?? 0))) },
        { reason: `before second Anxi (${row.rotationIndex})`, distance: 4, startTime: before(relative(row.startTime + Number(row.actions[secondAnxi]?.time ?? 0))) },
        { reason: `before first damage (${row.rotationIndex})`, distance: 2, startTime: before(relative(row.startTime + Number(row.actions[firstDamage]?.time ?? 0))) },
      );
    }
    ...
    burningIndexes.forEach((stepIndex, index) => {
      ...
      moves.push({ reason: `after Burning Heart section (${stepIndex})`, distance: 1, startTime: after(relative(row.startTime + row.effectiveCastTime)) });
    });

    moves.sort((left, right) => left.startTime - right.startTime);
    if (process.argv.includes("--patch")) {
      const moveLines = moves.map((move, index) => [
        "+    {",
        "+      \"type\": \"event\",",
        "+      \"event\": \"Move\",",
      ...
      patchSections.push(`*** Update File: ${path.slice(1)}\n@@\n     {\n       \"type\": \"event\",\n       \"event\": \"BattleEnd\",\n       \"startTime\": 60\n-    }\n+    },\n${moveLines}`);
    } else {
      ...
      moves.forEach((move, index) => {
        assert(actualMoves[index].startTime === move.startTime && actualMoves[index].distance === move.distance, `${rotation.name} Move ${index + 1} must be ${move.distance}m at ${move.startTime}s.`);
      });
      ...
      assert(firstSkillAction.row.actionStates[firstSkillAction.actionIndex]?.distance === 19, `${rotation.name} must begin its first skill action at 19m.`);
      assert(firstFleetingTrace.distance === 3, `${rotation.name} must begin its first Fleeting Trace at 3m.`);
      for (const row of skillRows.filter((candidate) => candidate.step.skill === "PhalanxbaneHeavyCharged3")) {
        const firstAnxi = row.actions.findIndex((action) => action.type === "trigger" && action.value === "AnxiSoldierBurningHeart2");
        const secondAnxi = row.actions.findIndex((action) => action.type === "trigger" && action.value === "AnxiSoldierBurningHeart3");
        const firstDamage = row.actions.findIndex((action) => action.type === "damage");
        assert(row.actionStates[firstAnxi]?.distance === 6, `${rotation.name} Burning Heart ${row.rotationIndex} first Anxi must trigger at 6m.`);
        assert(row.actionStates[secondAnxi]?.distance === 4, `${rotation.name} Burning Heart ${row.rotationIndex} second Anxi must trigger at 4m.`);
        assert(row.actionStates[firstDamage]?.distance === 2, `${rotation.name} Burning Heart ${row.rotationIndex} first damage must occur at 2m.`);
      }
