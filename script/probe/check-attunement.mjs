...
try {
  ...
  const damage = (attunement, skillTags) => calculateDamageBreakdown({ phyCoef: 1 }, {
    stats,
    attunement,
    skillTags,
    weapons: [],
  ...
  const twoMatching = damage({ ...baseAttunement, phalanxbaneChargedBoost: 0.06, phalanxbaneMartialBoost: 0.06 }, ["PhalanxbaneBlade", "Charged", "MartialArt"]);
  const penetrated = damage({ ...baseAttunement, physicalPenetration: 10 }, []);

  assert(closeTo(oneMatching / baseline, 1.06), "A matching armor attunement must apply as a standalone 1 + attunement DMG Bonus multiplier.");
  ...
  assert(closeTo(twoMatching / baseline, 1.12), "Matching armor attunement bonuses must sum inside the standalone multiplier.");
  assert(closeTo(penetrated / baseline, 1.05), "A weapon attunement without skill-match tags must apply to its penetration channel.");
