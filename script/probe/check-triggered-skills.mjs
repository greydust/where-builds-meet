...
const definitions = Object.assign({}, ...jsonFiles(path.join("data", "skill")).map((file) => JSON.parse(fs.readFileSync(file, "utf8"))));
const triggeredIds = new Set();

jsonFiles("data").forEach((file) => walk(JSON.parse(fs.readFileSync(file, "utf8")), (value) => {
...
Object.entries(definitions).forEach(([skillId, definition]) => {
  if (definition.tags?.includes("Triggered")) assert(triggeredIds.has(skillId), `${skillId} is tagged Triggered but is not referenced by a trigger action.`);
});
