...
try {
  ...
  assert(parsed[0].buildSetup.weaponSets.Cleftpeak === 2 && parsed[0].buildSetup.armorSets.Formbend === 0, "Legacy gearSets must migrate to weaponSets while armor sets receive their default.");
  assert(characterProfileMatches(parsed[0], { statOverrides: { minPhys: 123 }, attunementOverrides: { physicalPenetration: 0.051 }, innerWays: parsed[0].innerWays.map((row) => ({ ...row })), buildSetup: { ...parsed[0].buildSetup, innerWays: parsed[0].innerWays.map((row) => ({ ...row })), weaponSets: { ...parsed[0].buildSetup.weaponSets }, armorSets: { ...parsed[0].buildSetup.armorSets } } }), "Profile matching must include every profile-owned Main-tab selection.");

  const exported = JSON.parse(exportCharacterProfiles(parsed));
  assert(exported.version === 4 && exported.profiles[0].buildSetup.innerWays.length === 4 && exported.profiles[0].buildSetup.weaponSets.Cleftpeak === 2 && exported.profiles[0].buildSetup.armorSets.Formbend === 0 && !("food" in exported.profiles[0]) && !("divinecraft" in exported.profiles[0]) && !("globalDebuffs" in exported.profiles[0]), "Profile export v4 must include build-backed setup selections but omit independent session controls.");
