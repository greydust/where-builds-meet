...
try {
  ...
  const expectedSkills = [
    "FluteOfTheTides", "PhalanxbaneSpecial", "SnowpartingConversion", "SnowpartingQ",
    "SnowpartingHeavyVC", "SnowpartingLightCharged", "SnowpartingSpecial", "SnowpartingHeavyVC",
    "SnowpartingQStab", "PhalanxbaneQ", "SnowpartingConversion", "SnowpartingHeavyVC",
    "SnowpartingLightCharged", "SnowpartingHeavyVC", "SoaringSpin2", "SnowpartingLightCharged",
    "SnowpartingHeavyVC", "SnowpartingLightCharged", "SnowpartingHeavyVC", "LeapingToad", "Deflect",
    "PhalanxbaneSpecial", "SnowpartingConversion", "SnowpartingQ", "SnowpartingHeavyVC",
    "SnowpartingLightCharged", "SnowpartingSpecial", "SnowpartingHeavyVC", "SnowpartingQStab",
    "PhalanxbaneQ", "SnowpartingConversion", "SnowpartingHeavyVC", "SnowpartingLightCharged",
    "SnowpartingHeavyVC", "SnowpartingLightCharged", "SnowpartingHeavyVC", "SnowpartingLightCharged",
    "SnowpartingHeavyVC", "SnowpartingLightCharged", "SnowpartingHeavyVC", "FluteOfTheTidesCancel",
    "Deflect", "PhalanxbaneSpecial", "SnowpartingConversion", "SnowpartingQ", "SnowpartingHeavyVC",
    "SnowpartingLightCharged", "SnowpartingSpecial", "SnowpartingHeavyVC", "SnowpartingQStab",
    "PhalanxbaneQ", "SnowpartingConversion", "SnowpartingHeavyVC", "SnowpartingLightCharged",
    "SnowpartingHeavyVC", "SnowpartingLightCharged", "SnowpartingHeavyVC", "SnowpartingLightCharged",
    "SoaringSpin2",
  ...
  assert(rotation.start?.step === 4 && rotation.start.action === 0 && rotation.steps[4]?.skill === "SnowpartingQ", "The prepull slide hit must be the fight-start anchor.");
  assert(rotation.steps.some((step) => step.type === "event" && step.event === "Move" && step.distance === 19), "The rotation must begin at 19m.");
  assert(rotation.steps.some((step) => step.type === "event" && step.event === "Move" && step.distance === 3), "The first Fleeting Trace must begin at 3m.");
  const exhaustedIndex = rotation.steps.findIndex((step) => step.type === "event" && step.event === "Exhausted");
  assert(exhaustedIndex > 0 && rotation.steps[exhaustedIndex].after?.action === 4 && rotation.steps[exhaustedIndex + 1]?.skill === "SnowpartingLightCharged", "The dummy break must attach after Grave Frost's fourth damage action.");
  assert(rotation.steps.at(-1)?.event === "BattleEnd" && rotation.steps.at(-1)?.startTime === 60, "The dummy rotation must end at 60s.");
  ...
  const result = calculateRotationBaseline({
    timeline: {
      ...
      dots,
      effectDefinitions: { ...mysticBuffs, ...generalBuffs, ...stonesplitBuffs, ...generalDebuffs, ...stonesplitDebuffs, ...dots },
      innerWayConditions: [],
      innerWayRules: [],
      setupEffects: [],
  ...
  assert(result.metrics.totalDamage > 0 && result.duration === 60, "The translated preset must calculate as a 60-second rotation.");
  const firstPostBreakSpecialIndex = rotation.steps.findIndex((step, stepIndex) => stepIndex > exhaustedIndex && step.type === "skill" && step.skill === "SnowpartingSpecial");
  const firstPostBreakSpecial = result.timeline.find((row) => row.id === `rotation-${firstPostBreakSpecialIndex}`);
  assert(firstPostBreakSpecial?.actionStates[0]?.debuffs.some((effect) => effect.name === "Exhausted"), "The first post-break Fleeting Trace hit must see Exhausted.");
  if (process.argv.includes("--show-break-candidates")) {
    const candidates = result.timeline.flatMap((row) => row.kind === "rotation" && row.step.type === "skill"
      ? row.actions.map((action, actionIndex) => ({ rowId: row.id, skill: row.step.skill, action: actionIndex, time: row.startTime + Number(action.time ?? 0) - result.anchorTime, type: action.type })).filter((entry) => entry.time >= 23 && entry.time <= 27)
      : []);
