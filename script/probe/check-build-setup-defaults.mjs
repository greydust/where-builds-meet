...
try {
  ...
  assert(Object.keys(current).length === 0, "An explicitly saved empty override must use every setup value from the active build.");

  storage.delete("wwm-build-setup-overrides-v1");
  const migrated = loadBuildSetupOverrides(defaultBuildSetup);
  assert(migrated.innerWays?.[0]?.innerWay === "BreakingPoint", "The standalone Inner Way session must migrate only when the unified override has never been saved.");
