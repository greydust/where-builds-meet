...
try {
  const rotation = (
    await viteServer.ssrLoadModule("/data/rotation/stonesplit-strength/mixed-horse-tamer-standard.json")
  ).default;
  ...
  const postDragonCharged = result.timeline.filter(
    (row) =>
      row.kind === "rotation" &&
      row.step.type === "skill" &&
      row.step.skill === "PhalanxbaneHeavyCharged3" &&
  ...
  assert(
    surgingWaves?.type === "event" &&
      surgingWaves.event === "Buff" &&
      surgingWaves.buff === "SurgingWaves" &&
      surgingWaves.stack === 32 &&
      surgingWaves.before?.action === 8,
    "Dragon Head must receive 32 external Surging Waves stacks immediately before its damage hit.",
  );
  assert(
    hpEvent?.type === "event" &&
      hpEvent.event === "HP" &&
  ...
  assert(
    !dragonRow?.actionStates[0].buffs.some((effect) => effect.name === "SurgingWaves"),
    "External Surging Waves must not be present at Dragon Head's cast start.",
  );
  assert(
    dragonRow?.actionStates[8].buffs.find((effect) => effect.name === "SurgingWaves")?.stack === 40,
    "Dragon Head's eight applications must raise Surging Waves to 40 stacks before its hit.",
  );
  assert(
  ...
  assert(
    readableRotationText(
      ...
      0,
    ).includes("Dragon Head - Tide (break)"),
    "Readable format must mark externally exhausted Dragon Head as the break skill.",
  );
  assert(
    (fluteCast?.damageWithBuff ?? 0) > (fluteCast?.damage ?? 0) &&
      (fluteCast?.averageDpsWithBuff ?? 0) > (fluteCast?.averageDps ?? 0),
    "Horse Tamer's Flute cast must report inclusive buffed damage and DPS.",
  );
  assert(postDragonCharged.length >= 3, "The post-break slow charged and charged x2 sequence is missing.");
  assert(
    postDragonCharged[0].currentHPRatio === 1 && postDragonCharged[0].actionStates[0].currentHPRatio === 1,
    `HP must return to 100% at the first skill after Dragon Head (row=${postDragonCharged[0].currentHPRatio}, action=${postDragonCharged[0].actionStates[0].currentHPRatio}).`,
  );
  assert(
