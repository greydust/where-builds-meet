...
try {
  ...
  assert(resolveSegmentValue(segment, { actionTime: 1.5 }) === -0.7, "A value equal to the first threshold must use the first segment.");
  assert(resolveSegmentValue(segment, { actionTime: 2 }) === -1, "A value between thresholds must use the matching segment.");
  assert(resolveSegmentValue(segment, { actionTime: 3 }) === -1.2, "A value above every threshold must use the overflow segment.");

  const timeline = buildRotationTimeline({
    rotation: { name: "Segment timing probe", steps: [{ type: "skill", skill: "Probe" }] },
    skills: { Probe: { name: "Probe", castTime: 2, action: [{ type: "damage", time: 1.5 }, { type: "damage", time: 2 }], modifier: [{ effect: { castTimeModifier: { function: "segment", param1: "actionTime", param2: [1.5], param3: [-0.7, -1] } } }], tags: [] } },
