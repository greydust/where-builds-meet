...
function StatField({ definition, stats, onChange, onReset, modified = false, derivedLabel, derivedValue, derivedUnit, compact }: {
  ...
  return (
    <label className={`field ${compact ? "compact-field" : ""} ${modified ? "modified-field" : ""}`}>
      <span className="field-label"><span>{definition.label}{definition.unit && definition.showUnitInLabel !== false ? ` ${definition.unit}` : ""}</span>{modified && <button className="stat-reset-button" type="button" aria-label={`Reset ${definition.label}`} title="Reset to calculated value" onClick={(event) => { event.preventDefault(); onReset?.(); }}><UiIcon name="reset" /></button>}</span>
  ...
  return (
    <>
      <div className="app-layout">
          ...
          <div className="attunement-list">
            {attunementFields.map(([key, label, unit], index) => (
              <label className={`attunement-field ${index === armorAttunementStart ? "attunement-section-start" : ""} ${Object.prototype.hasOwnProperty.call(attunementOverrides, key) ? "modified-field" : ""}`} key={key}>
                <span className="attunement-label"><span>{label}</span>{Object.prototype.hasOwnProperty.call(attunementOverrides, key) && <button className="stat-reset-button" type="button" aria-label={`Reset ${label}`} title="Reset to calculated value" onClick={(event) => { event.preventDefault(); resetAttunement(key); }}><UiIcon name="reset" /></button>}</span>
        ...
        <section className="middle-stats-column">
          <section className="panel inner-way-panel">
            <div className="panel-heading"><div><h2>Inner Ways</h2></div>{buildSetupOverrides.innerWays && <button className="stat-reset-button" type="button" aria-label="Reset Inner Ways" title="Reset to build value" onClick={() => onBuildSetupReset("innerWays")}><UiIcon name="reset" /></button>}</div>
          ...
          <section className="panel setup-placeholder-panel">
            <div className="panel-heading"><div><h2>Arsenal</h2></div>{buildSetupOverrides.arsenal !== undefined && <button className="stat-reset-button" type="button" aria-label="Reset Arsenal" title="Reset to build value" onClick={() => onBuildSetupReset("arsenal")}><UiIcon name="reset" /></button>}</div>
      ...
      <dialog className="character-profile-dialog" ref={profileDialogRef} onCancel={() => setProfileTransferStatus(undefined)}>
        <div className="character-profile-dialog-heading"><div><h2>Character Profiles</h2><p>Profiles save modified stats and the complete Main-tab setup.</p></div><button className="icon-button" type="button" aria-label="Close character profiles" onClick={() => profileDialogRef.current?.close()}><UiIcon name="close" /></button></div>
    ...
    <div className="requirement-editor">
      ...
      {requirements.map((requirement, index) => {
        ...
        return <div className="condition-row" key={index}>
          ...
          <button type="button" aria-label="Remove condition" onClick={() => remove(index)}><UiIcon name="close" /></button>
        </div>;
      })}
    </div>
  );
...
function ModifierDetails({ item, onChange }: { item: EditableObject; onChange: (item: EditableObject) => void }) {
  ...
  return <div className="structured-detail">
    ...
    <div className="sub-editor-heading"><span>Effects</span><button className="button button-small" type="button" onClick={addEffect}>Add effect</button></div>
      ...
      <button type="button" aria-label="Remove effect" onClick={() => removeEffect(field)}><UiIcon name="close" /></button>
    </div>)}
  </div>;
}
...
function EffectRuleDetails({ item, onChange }: { item: EditableObject; onChange: (item: EditableObject) => void }) {
  ...
  return <div className="structured-detail">
    ...
    {effectEntries.map(([field, value]) => <div className="effect-row effect-rule-row" key={field}>
      ...
      <EffectValueEditor value={value} onChange={(nextValue) => updateEffect(field, nextValue)} />
      <button type="button" aria-label={`Remove ${field}`} onClick={() => removeEffect(field)}><UiIcon name="close" /></button>
    </div>)}
  </div>;
}
            ...
            <div className={`array-item ${expanded === index ? "expanded" : ""}`} key={index}>
              <div className="array-item-header">
                ...
                <div className="array-item-controls">
                  <button type="button" aria-label="Move up" disabled={index === 0} onClick={() => moveItem(index, -1)}><UiIcon name="up" /></button>
                  <button type="button" aria-label="Move down" disabled={index === items.length - 1} onClick={() => moveItem(index, 1)}><UiIcon name="down" /></button>
                  <button type="button" aria-label="Delete" onClick={() => deleteItem(index)}><UiIcon name="close" /></button>
                </div>
              </div>
...
function StackEffectsEditor({ groups, onChange, skillIds }: { groups: EditableObject[][]; onChange: (groups: EditableObject[][]) => void; skillIds: string[] }) {
  ...
  return <section className="array-editor stack-effects-editor">
    ...
    <div className="array-editor-list">{groups.map((group, index) => <div className={`array-item ${expanded === index ? "expanded" : ""}`} key={index}>
      <div className="array-item-header"><button className="array-item-toggle" type="button" onClick={() => setExpanded(expanded === index ? null : index)}>Stack {index + 1} · {group.length} effect{group.length === 1 ? "" : "s"}</button><div className="array-item-controls"><button type="button" aria-label="Move stack up" disabled={index === 0} onClick={() => moveGroup(index, -1)}><UiIcon name="up" /></button><button type="button" aria-label="Move stack down" disabled={index === groups.length - 1} onClick={() => moveGroup(index, 1)}><UiIcon name="down" /></button><button type="button" aria-label="Delete stack" onClick={() => { onChange(groups.filter((_, groupIndex) => groupIndex !== index)); setExpanded(null); }}><UiIcon name="close" /></button></div></div>
...
function RotationEditorTab({ character, onMetricsChange, onActiveSimulationBundleChange }: { character: CharacterState; onMetricsChange: (metrics: RotationMetrics, isActive: boolean) => void; onActiveSimulationBundleChange: (bundle: RotationSimulationBundle, rotationName: string, bundleKey: string) => void }) {
  ...
  return <section className="panel rotation-editor-panel"><div className="rotation-editor-layout">
    ...
    <div className="skill-detail-heading"><div>{editingName && !rotationLocked ? <input className="rotation-name-input" autoFocus value={rotation.name} onChange={(event) => setRotation({ ...rotation, name: event.target.value })} onBlur={() => setEditingName(false)} onKeyDown={(event) => { if (event.key === "Enter") setEditingName(false); }} /> : <h3>{rotation.name || "Unnamed Rotation"}{!rotationLocked && <button className="icon-button" type="button" aria-label="Edit rotation name" onClick={() => setEditingName(true)}><UiIcon name="edit" /></button>}</h3>}{rotationLocked && <p className="rotation-default-note">This is a prebuilt default rotation and cannot be changed. Duplicate it to edit.</p>}</div><div className="detail-active-actions">{status && <span className="editor-status">{status}</span>}<span className="rotation-heading-actions"><button className="button button-secondary button-small" type="button" onClick={duplicateRotation}>Duplicate</button>{!rotationLocked && <button className="button button-primary button-small" type="button" onClick={save}>Save</button>}<button className="button button-small detail-active-button" type="button" disabled={editingRotationId === activeRotationId} onClick={() => activateRotation(editingRotationId)}>{editingRotationId === activeRotationId ? "Active" : "Make Active"}</button></span></div></div>
    ...
    <div className="rotation-scroll-content" ref={rotationScrollRef}><div className={`rotation-table ${showDistanceColumn ? "show-distance" : ""} ${showHPColumn ? "show-hp" : ""}`}>
      <div className="rotation-table-header"><span></span><span>#</span><span>Start Time</span><span>Cast Time</span><span>Skill</span>{showDistanceColumn && <span>Distance</span>}{showHPColumn && <span>Self HP</span>}<span className="rotation-damage-heading">Damage</span><span>Buff</span><span>Debuff</span><span>Actions</span></div>
      <div className="rotation-step-list">
        {displayEntries.map((entry, index) => {
          ...
          const effectNames = (effects: Array<{ name: string; stack?: number; maxStack?: number; expiresAt?: number }>, atTime: number) => effects.length === 0 ? "" : <span className="effect-plates">{effects.map((effect) => {
            ...
            const timeLeft = effect.expiresAt === undefined ? "∞" : Math.max(0, effect.expiresAt - atTime).toFixed(2);
            const plateKind = dotEffectIds.has(effect.name) ? " effect-plate-dot" : generalDebuffIds.has(effect.name) ? " effect-plate-general-debuff" : "";
            return <span className={`effect-plate${plateKind}`} key={`${effect.name}-${effect.stack ?? 1}`}>
              {label}
              <span className="effect-plate-tooltip" role="tooltip"><strong>{name} - {timeLeft}s left</strong>{description && <span>{description}</span>}</span>
            </span>;
          })}</span>;
          ...
          return <div className="rotation-row-group" key={`${row.id}-${entry.kind}-${actionIndex ?? "skill"}`}>
            {!isAction && <div className={`rotation-table-row ${isManualEvent ? "rotation-event-row" : ""} ${isManualEvent && step.event === "Move" ? "rotation-move-event-row" : ""}`} data-rotation-step-index={row.rotationIndex}>
              ...
              <span>{isManualEvent && step.event === "Buff" ? rotationLocked ? <span>{manualBuffDefinitions[step.buff]?.name ?? step.buff}{(step.stack ?? 1) > 1 ? ` ×${step.stack}` : ""}</span> : <span className={`rotation-effect-event-control ${(effectDefinitions[step.buff]?.maxStack ?? 1) <= 1 ? "single" : ""}`}><select className="rotation-effect-select" aria-label="Buff to apply" value={step.buff} onChange={(event) => { const buff = event.target.value; updateStep(row.rotationIndex ?? 0, { buff, stack: Math.min(step.stack ?? 1, effectDefinitions[buff]?.maxStack ?? 1) }); }}>{Object.entries(manualBuffDefinitions).map(([id, definition]) => <option value={id} key={id}>{definition.name ?? id}</option>)}</select>{(effectDefinitions[step.buff]?.maxStack ?? 1) > 1 && <input className="rotation-effect-stack" aria-label="Buff stacks" type="number" min="1" max={effectDefinitions[step.buff]?.maxStack} step="1" value={step.stack ?? 1} onChange={(event) => { const stack = Number(event.target.value); if (Number.isFinite(stack)) updateStep(row.rotationIndex ?? 0, { stack: Math.max(1, Math.min(effectDefinitions[step.buff]?.maxStack ?? 1, Math.floor(stack))) }); }} />}</span> : isManualEvent ? "" : effectNames(row.buffs, startTime)}</span>
              <span>{isManualEvent && step.event === "Debuff" ? rotationLocked ? <span>{manualDebuffDefinitions[step.debuff]?.name ?? step.debuff}{(step.stack ?? 1) > 1 ? ` ×${step.stack}` : ""}</span> : <span className={`rotation-effect-event-control ${(effectDefinitions[step.debuff]?.maxStack ?? 1) <= 1 ? "single" : ""}`}><select className="rotation-effect-select" aria-label="Debuff to apply" value={step.debuff} onChange={(event) => { const debuff = event.target.value; updateStep(row.rotationIndex ?? 0, { debuff, stack: Math.min(step.stack ?? 1, effectDefinitions[debuff]?.maxStack ?? 1) }); }}>{Object.entries(manualDebuffDefinitions).map(([id, definition]) => <option value={id} key={id}>{definition.name ?? id}</option>)}</select>{(effectDefinitions[step.debuff]?.maxStack ?? 1) > 1 && <input className="rotation-effect-stack" aria-label="Debuff stacks" type="number" min="1" max={effectDefinitions[step.debuff]?.maxStack} step="1" value={step.stack ?? 1} onChange={(event) => { const stack = Number(event.target.value); if (Number.isFinite(stack)) updateStep(row.rotationIndex ?? 0, { stack: Math.max(1, Math.min(effectDefinitions[step.debuff]?.maxStack ?? 1, Math.floor(stack))) }); }} />}</span> : isManualEvent ? "" : effectNames(row.debuffs, startTime)}</span>
              <span className="rotation-controls">{isAttachedEvent && <><span className="rotation-control-placeholder" aria-hidden="true" /><button type="button" aria-label="Move event to previous action" disabled={rotationLocked || attachedTargetIndex <= 0} onClick={(event) => moveAttachedEvent(row.rotationIndex ?? 0, -1, event.currentTarget)}><UiIcon name="up" /></button><button type="button" aria-label="Move event to next action" disabled={rotationLocked || attachedTargetIndex < 0 || attachedTargetIndex >= availableAttachmentTargets.length - 1} onClick={(event) => moveAttachedEvent(row.rotationIndex ?? 0, 1, event.currentTarget)}><UiIcon name="down" /></button></>}{row.kind === "rotation" && !isManualEvent && <button className="rotation-expand-button" type="button" aria-label={`${actionsExpanded ? "Collapse" : "Expand"} ${skillDisplayName(skill, stepSkill ?? "skill")} actions`} aria-expanded={actionsExpanded} onClick={() => toggleSkillActions(row.id)}><UiIcon name={actionsExpanded ? "chevronDown" : "chevronRight"} /></button>}{row.kind === "rotation" && (!isManualEvent || isDelayEvent) && <><button type="button" aria-label="Move up" disabled={rotationLocked || (row.rotationIndex ?? 0) === 0} onClick={() => moveStep(row.rotationIndex ?? 0, -1)}><UiIcon name="up" /></button><button type="button" aria-label="Move down" disabled={rotationLocked || (row.rotationIndex ?? 0) === rotation.steps.length - 1} onClick={() => moveStep(row.rotationIndex ?? 0, 1)}><UiIcon name="down" /></button></>} {row.kind === "rotation" && <><button type="button" aria-label="Delete step" disabled={rotationLocked} onClick={() => removeStep(row.rotationIndex ?? 0)}><UiIcon name="close" /></button>{(!isManualEvent || isDelayEvent) && <button type="button" aria-label="Add step below" disabled={rotationLocked} onClick={() => addStepBelow(row.rotationIndex ?? 0)}><UiIcon name="plus" /></button>}</>}{isAttachedEvent && <span className="rotation-control-placeholder" aria-hidden="true" />}</span>
            </div>
            }
            {isAction && (() => {
  ...
  <dialog className="rotation-readable-dialog" ref={readableDialogRef} onClose={() => setReadableDialogOpen(false)}>
    <div className="rotation-readable-heading"><div><span className="detail-kicker">Readable Format</span><h3>{rotation.name || "Unnamed Rotation"}</h3></div><button className="icon-button" type="button" aria-label="Close readable rotation" onClick={() => readableDialogRef.current?.close()}><UiIcon name="close" /></button></div>
