...
export function readableRotationText(timeline: TimelineRow[], startAnchor: { rowId: string; actionIndex?: number }, anchorTime: number) {
  ...
  const breakTimes = timeline.filter((row) => row.kind === "rotation" && row.step.type === "event" && row.step.event === "Exhausted" && !row.skipped).map((row) => row.startTime);
  breakTimes.forEach((breakTime) => {
    const containingRow = skillRows.find((row) => breakTime >= row.startTime - 1e-6 && breakTime <= row.startTime + row.effectiveCastTime + 1e-6);
