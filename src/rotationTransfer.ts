...
function parseRotationStep(value: unknown): RotationStep | undefined {
  ...
  if (step.type === "event" && step.event === "Exhausted" && (after || before)) {
    return { type: "event", event: "Exhausted", after: after ?? before!, ...(typeof step.duration === "number" && Number.isFinite(step.duration) ? { duration: Math.max(0, step.duration) } : {}) };
  }
  ...
  if (step.type === "event" && step.event === "HP" && before && typeof step.currentHPRatio === "number" && Number.isFinite(step.currentHPRatio)) {
